...
const Followings = () => {
  ...
  // Show loading message if followings is undefined
      ...
      {followings.length > 0 ? (
        <div className='user-followings-section-div-insides'>
          {followings.map((following, index) => (
            <div key={following._id || index} className='user-followings-sect-cornels'>
              <div className='user-followings-sect-cornels-insides'>
                ...
                <div className='followings-p-h3-text-section'>
                  <h3 className='followings-name-h3'>{following.name || "No Name"}</h3>
                  <p className='followings-p-email'>{following.email || "No Email"}</p>

                  {/* Optional created_at timestamp */}
                  {following.created_at && (
                    <div className='followings-timestamps'>
                      <p>{new Date(following.created_at).toLocaleDateString()}</p>
                      <p>{new Date(following.created_at).toLocaleTimeString()}</p>
                    </div>
                  )}
                </div>
